...
int up_start = 0;  //一键启动
int start=0;
uint8_t Shake_shoulder = 0 ;//抖肩

// 遥控器开关拨动 判断是否开启机器人跳动和开启roll自稳
void remote_switch()
{
  // if( ppmValues[7] < 1300)  //遥控器左上波轮滑到最下面（L），机器人跳跃
  // {
  ...
  //通道五右上角
  if (ppmValues[7] > 1600)//机器人轮毂电机启动和机器人开启自稳定  最下面
  {
    EH_rollflag = 1;
    up_start =0 ;

  }
  else if (ppmValues[7] < 1600 && ppmValues[7] > 1400)//机器人轮毂电机启动和机器人关闭自稳定
  {
    EH_rollflag = 0;
    up_start = 1;
  }
  else if (ppmValues[7] < 1400)//机器人关闭轮毂电机和机器人关闭自稳定 最上面
  {
    EH_rollflag = 0;
    up_start = 0;

  }
  // Serial.print(ppmValues[6]);
  // Serial.print(",");
...
void storeFilteredPPMData()
{
  ...
  // Serial.print(",");
  // Serial.print(ppmValues[2]);
  // Serial.print(",");
  // Serial.print(ppmValues[3]);
  // Serial.print(",");
  // Serial.print(ppmValues[4]);
  // Serial.print(",");        
  // Serial.print(ppmValues[5]);
  // Serial.print(",");
  // Serial.print(ppmValues[6]);
  // Serial.print(",");
  // Serial.println(ppmValues[7]);
